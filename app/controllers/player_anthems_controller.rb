class PlayerAnthemsController < ApplicationController
    def index
        # render all instances
        render json: PlayerAnthem.all, status: :ok
    end


    def show
        # render one instances
        item = PlayerAnthem.find(params[:id])
        render json: item, status: 200
    end

    def create
        new_item = PlayerAnthem.create!(item_params.except(:id))
