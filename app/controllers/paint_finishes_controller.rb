class PaintFinishesController < ApplicationController
    def index
        # render all instances
        render json: PaintFinish.all, status: :ok
    end


    def show
        # render one instances
        item = PaintFinish.find(params[:id])
        render json: item, status: 200
    end

    def create
        new_item = PaintFinish.create!(item_params.except(:id))
