class AvatarBordersController < ApplicationController
    def index
        # render all instances
        render json: AvatarBorder.all, status: :ok
    end


    def show
        # render one instances
        item = AvatarBorder.find(params[:id])
        render json: item, status: 200
    end

    def create
        new_item = AvatarBorder.create!(item_params.except(:id))
